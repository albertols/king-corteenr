...
object ML_CH07 extends SparkUtils {
...
//  val rmind = new IndexedRowMatrix(rm.rows.zipWithIndex().map(x => IndexedRow(x._2, x._1)))
  ...
  //Section 7.4
  val housingLines = sc.textFile(getClass.getResource("/ml/housing.data").getPath, 6)
  ...
  def printMat(mat: BM[Double]) = {
    ...
    for (i <- 0 to mat.rows - 1) {
      print("%-6d".format(i)); for (j <- 0 to mat.cols - 1) print(" %+9.3f".format(mat(i, j))); println
    }
  }
  ...
  */
  ...
  val housingData = housingVals.map(x => {
    val a = x.toArray; LabeledPoint(a(a.length - 1), Vectors.dense(a.slice(0, a.length - 1)))
  })
  ...
  //Section 7.5.4
  model.save(sc, "trained/model") // TODO

  import org.apache.spark.mllib.regression.LinearRegressionModel

  val model2 = LinearRegressionModel.load(sc, "trained/model") // TODO
